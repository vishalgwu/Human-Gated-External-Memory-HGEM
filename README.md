...
Splits:
  ...
  Holdout: remaining 21,722 total
...
Important status note: the generated constraint files are extraction seeds only. They are marked `needs_subject_matter_review` and must not be treated as validated RDI/CVR ground truth yet.
...
- Confirm whether the MATH PRM annotation source will be integrated from Lightman/PRM800K or replaced by expert review.
...
Status: JSON skeletons created; claims not written.

Done:

- Created empty adversarial bank skeletons for GSM8K, MATH, and SciBench.
...
```text
Validate pilot/main constraint sets and write adversarial false-claim banks.
```
