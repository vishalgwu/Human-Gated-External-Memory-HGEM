...
The repo is on `main`. The folder-structure setup was merged into `main` from the previous branch.
...
- Download SciBench.
- Store raw data under `02_DATASETS/raw/`.
- Convert each benchmark into the standard processed JSON format.
- Record dataset source, date, file sizes, and problem counts.
- Keep raw data read-only after download.
...
- Build constraint files for GSM8K.
- Build constraint files for MATH.
- Build constraint files for SciBench.
- Have subject-matter review for constraints.
...
- Create pilot split: 10 problems per benchmark.
- Create main split: 40 problems per benchmark.
- Create holdout split from remaining problems.
- Stratify by difficulty where available.
- Commit split metadata, not private/generated data if ignored.
...
- Create plausible false claims for GSM8K.
- Create plausible false claims for MATH.
- Create plausible false claims for SciBench.
- Link each false claim to a specific target constraint.
...
The next concrete task is Step 2:

```text
Download and prepare GSM8K, MATH Dataset, and SciBench.
```

Before downloading data, decide whether large/raw datasets should remain local-only or be tracked with Git LFS / external storage. Current `.gitignore` is configured to keep raw and generated dataset files out of normal Git commits.
